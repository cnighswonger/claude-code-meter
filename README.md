...
**Recommended collector for v0.8.0+:** claude-code-cache-fix `>= 4.3.0`. v0.8.0 adds schema acceptance for the optional `agent_id` + `agent_id_source` fields (default-off gate on the cache-fix side: `CACHE_FIX_USAGE_LOG_AGENT_ID=on`). Per upstream [CC#66761](https://github.com/anthropics/claude-code/issues/66761), Claude Code sets the canonical `x-claude-code-agent-id` header on Task/Agent-tool subagents but NOT on Workflow-tool–spawned subagents — operators running fan-out workflows (`agent()`, `parallel()`, `pipeline()`) cannot attribute per-Workflow-leg cost without those fields. The cache-fix proxy derives a stable per-leg id when CC's canonical header is absent and tags the provenance via `agent_id_source` (`"cc_header"` vs. `"cache_fix_derived"`). **Operator attestation contract**: flipping the env-var without upgrading meter produces rows that fail validation — the visible symptom is a nonzero `skipped=` counter in `claude-meter ingest` tick output. Verify your meter version before flipping the env-var.
